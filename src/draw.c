 ...
 */
static void
set_object_color (DrawAPI *dapi, AnyObjectType *obj,
...
set_color_for_pin (DrawAPI *dapi, PinType *pin)
{
  if (dapi->doing_pinout)
    dapi->graphics->set_color (dapi->fg_gc, PCB->PinColor);
  else
    set_object_color (dapi, (AnyObjectType *)pin, PCB->WarnColor,
...
{
  if (dapi->doing_pinout)
    dapi->graphics->set_color (dapi->fg_gc, PCB->ViaColor);
  else
    set_object_color (dapi, (AnyObjectType *)via, PCB->WarnColor,
                      PCB->ViaSelectedColor, PCB->ConnectedColor, PCB->ViaColor);
}
...
set_color_for_pad (DrawAPI *dapi, LayerType *layer, PadType *pad)
{
  if (dapi->doing_pinout)
    ...
    set_object_color (dapi, (AnyObjectType *)pad, PCB->WarnColor,
                      PCB->PinSelectedColor, PCB->ConnectedColor,
                      FRONT (pad) ? PCB->PinColor : PCB->InvisibleObjectsColor);
}

static void
set_layer_object_color (DrawAPI *dapi, LayerType *layer, AnyObjectType *obj)
{
  set_object_color (dapi, obj, NULL, layer->SelectedColor, PCB->ConnectedColor, layer->Color);
}
...
hole_callback (const BoxType * b, void *cl)
{
  ...
  if (TEST_FLAG (HOLEFLAG, pv))
    {
      set_object_color (dapi, (AnyObjectType *) pv,
...
{
  set_layer_object_color (dapi, layer, (AnyObjectType *) line);
...
rat_callback (const BoxType * b, void *cl)
{
  ...
  set_object_color (dapi, (AnyObjectType *) rat, NULL, PCB->RatSelectedColor,
...
{
  set_layer_object_color (dapi, layer, (AnyObjectType *) arc);
  _draw_arc (dapi, arc);
}
...
poly_callback (const BoxType * b, void *cl)
{
  ...
  set_layer_object_color (dapi, info->layer, (AnyObjectType *) polygon);
