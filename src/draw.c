...
#endif
      dapi->draw_layer (LAYER_PTR (max_copper_layer + side), NULL); // drawn_area);
      /* draw package */
      r_search (PCB->Data->element_tree, drawn_area, NULL, element_callback, &side);
      r_search (PCB->Data->name_tree[NAME_INDEX (PCB)], drawn_area, NULL, name_callback, &side);
#if 0
  ...
  if (gui->poly_after)
    {
      ...
      dapi->draw_layer (LAYER_PTR (max_copper_layer + layer), NULL); // drawn_area);
      /* draw package */
      r_search (PCB->Data->element_tree, drawn_area, NULL, element_callback, &side);
      r_search (PCB->Data->name_tree[NAME_INDEX (PCB)], drawn_area, NULL, name_callback, &side);
    }
  ...
  for (i = n_entries - 1; i >= 0;
      i--, gui->set_layer (0, group, 0)) /* HACK: Subcomposite each layer in a layer group separately */
    {
      ...
      if (layernum < max_copper_layer && Layer->On)
        dapi->draw_layer (Layer, NULL); //drawn_area, true);
    }
  if (n_entries > 1)
    rv = 1;
  return rv;
