...
static const char fontedit_syntax[] =
"FontEdit()";
...
FontEdit (int argc, char **argv, int Ux, int Uy)
{
  ...
  for (s=0; s<=MAX_FONTPOSITION; s++)
      ...
      for (l=0; l<symbol->LineN; l++)
	{
	  CreateDrawnLineOnLayer (lfont,
				  symbol->Line[l].Point1.X + ox, symbol->Line[l].Point1.Y + oy,
				  symbol->Line[l].Point2.X + ox, symbol->Line[l].Point2.Y + oy,
  ...
  for (l=0; l<16; l++)
    {
      int x = (l+1) * CELL_SIZE;
      CreateDrawnLineOnLayer (lgrid, x, 0, x, PCB->MaxHeight, 100, 100, NoFlags());
    }
  for (l=0; l<= MAX_FONTPOSITION / 16 + 1; l++)
    {
      int y = (l+1) * CELL_SIZE;
      CreateDrawnLineOnLayer (lgrid, 0, y, PCB->MaxWidth, y, 100, 100, NoFlags());
    }
  return 0;
}

static const char fontsave_syntax[] =
"FontSave()";

static const char fontsave_help[] =
...
FontSave (int argc, char **argv, int Ux, int Uy)
{
  ...
  for (i=0; i < lfont->LineN; i++)
	...
	symbol->Width = x2;
      ...
      CreateNewLineInSymbol (symbol,
			     x1, y1, x2, y2, l->Thickness);
    }
