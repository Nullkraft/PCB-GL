/* $Id: main.c,v 1.42 2006-03-22 22:49:29 danmc Exp $ */
...
RCSID("$Id: main.c,v 1.42 2006-03-22 22:49:29 danmc Exp $");
...
usage (void)
{
  ...
  for (i=0; hl[i]; i++)
    {
      if (hl[i]->gui) n_gui++;
      if (hl[i]->printer) n_printer++;
      if (hl[i]->exporter) n_exporter++;
    }
...
HID_Attribute main_attribute_list[] = {
  ...
  {"verbose", "Be verbose", HID_Boolean, 0, 0, { 0, 0, 0 }, 0, &Settings.verbose },
  {"copyright", "Show Copyright", HID_Boolean, 0, 0, { 0, 0, 0 }, 0, &show_copyright },
  {"show-defaults", "Show option defaults", HID_Boolean, 0, 0, { 0, 0, 0 }, 0, &show_defaults },
  {"show-actions", "Show option defaults", HID_Boolean, 0, 0, { 0, 0, 0 }, 0, &show_actions },
  ...
  COLOR(BackgroundColor, "#e5e5e5", "background-color", "color for background"),
  COLOR(CrosshairColor, "#ff0000", "crosshair-color", "color for the crosshair"),
  COLOR(CrossColor, "#cdcd00", "cross-color", "color for cross"),
  COLOR(ViaColor, "#7f7f7f", "via-color", "color for vias"),
  COLOR(ViaSelectedColor, "#00ffff", "via-selected-color", "color for selected vias"),
  COLOR(PinColor, "#4d4d4d", "pin-color", "color for pins"),
  COLOR(PinSelectedColor, "#00ffff", "pin-selected-color", "color for selected pins"),
  COLOR(PinNameColor, "#ff0000", "pin-name-color", "color for pin names and numbers"),
  COLOR(ElementColor, "#000000", "element-color", "color for elements"),
  COLOR(RatColor, "#b8860b", "rat-color", "color for "),
  COLOR(InvisibleObjectsColor, "#cccccc", "invisible-objects-color", "color for invisible objects"),
  COLOR(InvisibleMarkColor, "#cccccc", "invisible-mark-color", "color for invisible marks"),
  COLOR(ElementSelectedColor, "#00ffff", "element-selected-color", "color for selected elements"),
  COLOR(RatSelectedColor, "#00ffff", "rat-selected-color", "color for selected rats"),
  COLOR(ConnectedColor, "#00ff00", "connected-color", "color for connections"),
  COLOR(OffLimitColor, "#cccccc", "off-limit-color", "color for off-limits areas"),
  ...
  ISET(LineThickness, 1000, "line-thickness", "Initial thickness of new lines."),
  ...
  SSET(FontCommand, "M4PATH='%p';export M4PATH;echo 'include(%f)' | " GNUM4, "font-command", 0),
  SSET(FileCommand, "cat '%f'", "file-command", "Command to read a file."),
  SSET(ElementCommand, "M4PATH='%p';export M4PATH;echo 'include(%f)' | " GNUM4, "element-command", 0),
  SSET(PrintFile, "%f.output", "print-file", 0),
  SSET(LibraryCommandDir, PCBLIBDIR, "lib-command-dir", 0),
  SSET(LibraryCommand, PCBLIBDIR "/QueryLibrary.sh '%p' '%f' %a", "lib-command", 0),
  SSET(LibraryContentsCommand, PCBLIBDIR "/ListLibraryContents.sh '%p' '%f'", "lib-contents-command", 0),
  ...
  SSET(FontFile, "default_font", "default-font", "File name of default font."),
  SSET(Groups, "1,c:2,s:3:4:5:6:7:8", "groups", 0),
  SSET(Routes, "Signal,1000,3600,2000,1000:Power,2500,6000,3500,1000"
       ":Fat,4000,6000,3500,1000:Skinny,600,2402,1181,600", "route-styles", 0),
  ...
  SSET(ScriptFilename, 0, "action-script", "If set, this file is executed at startup."),
  SSET(ActionString, 0, "action-string", "If set, this is executed at startup."),
