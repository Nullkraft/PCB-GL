...
static Widget fsb = 0;
static XmString xms_pcb, xms_net, xms_vend, xms_all, xms_load, xms_loadv, xms_save;
...
DoWindows(int argc, char **argv, int x, int y)
{
  ...
  if (strcmp(a, "1") == 0
      || strcasecmp(a, "Layout") == 0)
    {
    }
  else if (strcmp(a, "2") == 0
	   || strcasecmp(a, "Library") == 0)
    {
      ...
      for (i=0; hids[i]; i++)
	{
	  if (hids[i]->exporter)
	      ...
	      else
		{
	      ...
	      stdarg(XmNrightAttachment, XmATTACH_FORM);
	      stdarg(XmNleftAttachment, XmATTACH_FORM);
	      Widget w = XmCreatePushButton(selector, (char *)hids[i]->name, args, n);
	      XtManageChild(w);
	      XtAddCallback(w, XmNactivateCallback, (XtCallbackProc)dialog_callback, (XtPointer)(i + 1));
	      prev = w;
	    }
	}
	  ...
	  for (j=0; j<MAX_LAYER; j++)
	    {
	      ...
	      lgbuttons[i][j] = XmCreateToggleButton(buttonform, "label", args, n);
