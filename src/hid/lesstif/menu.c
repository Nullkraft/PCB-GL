...
LayersChanged (int argc, char **argv, int x, int y)
{
  ...
  if (lesstif_m_layer)
	...
	{
	case LB_RATS: name = "Rats"; break;
	case LB_SILK: name = "Silk"; break;
	default: name = PCB->Data->Layer[current_layer].Name; break;
	}
      n = 0;
...
static void
layerpick_button_callback (Widget w, int layer, XmPushButtonCallbackStruct *pbcs)
{
  int l, i;
  char *name;
  ...
  switch (layer)
    ...
    case LB_RATS: name = "Rats"; break;
    case LB_SILK: name = "Silk"; break;
    default: name = PCB->Data->Layer[layer].Name; break;
    }
  n = 0;
  stdarg(XmNbackground, fg_colors[layer]);
  stdarg(XmNforeground, bg_color);
  ...
  for (i = 0; i < LB_NUM; i ++)
    {
      ...
      switch (i) {
      case LB_SILK: name = "Silk"; break;
      case LB_RATS: name = "Rat Lines"; break;
      case LB_PINS: name = "Pins/Pads"; break;
      case LB_VIAS: name = "Vias"; break;
      case LB_BACK: name = "Far Side"; break;
      case LB_MASK: name = "Solder Mask"; break;
      }
      n = 0;
      if (i < MAX_LAYER)
	{
...
insert_layerpick_buttons (Widget *menu)
{
  ...
  for (i = 0; i < LB_NUMPICK; i ++)
      ...
      if (i < MAX_LAYER)
	{
      ...
      XtManageChild(btn);
      XtAddCallback(btn, XmNvalueChangedCallback,
		    (XtCallbackProc)layerpick_button_callback, (XtPointer)i);
      lb->w[i] = btn;
    }
  lb->is_pick = 1;
  LayersChanged (0, 0, 0, 0);
...
note_widget_flag(Widget w, char *type, char *name)
{
  if (n_wflags >= max_wflags)
    {
      max_wflags += 20;
      wflags = MyRealloc(wflags, max_wflags * sizeof(WidgetFlagType), __FUNCTION__);
    }
  wflags[n_wflags].w = w;
...
static void
radio_callback(Widget toggle, ToggleItem *me, XmToggleButtonCallbackStruct *cbs)
{
  ...
  for (i=0; i<acc_num; i++)
    {
      ...
      printf("%16s%s\t", mod, 
	     acc_table[i].key_char ? ch : XKeysymToString(acc_table[i].key));
  ...
  if (acc_num == acc_max)
    {
      acc_max += 20;
      if (acc_table)
	acc_table = (acc_table_t *) realloc (acc_table, acc_max * sizeof(acc_table_t));
      else
...
lesstif_key_event (XKeyEvent *e)
{
  ...
  for (i=0; i<acc_num; i++)
      ...
      if (acc_table[i].mods == (mods & ~M_Shift))
	{
	  //printf("mods match 1\n");
	  if (slen == 1
	      && buf[0] == acc_table[i].key_char)
	    break;
	  if (sym == acc_table[i].key)
	    break;
	}
      if (mods & M_Shift
	  && acc_table[i].mods == mods)
	{
	  //printf("mods match 2\n");
	  if (slen2 == 1
	      && buf2[0] == acc_table[i].key_char)
	    break;
  ...
  for (vi=1; vi<acc_table[i].node->c; vi++)
    if (resource_type(acc_table[i].node->v[vi]) == 10)
      if (hid_parse_actions (acc_table[i].node->v[vi].value, lesstif_call_action))
	break;
  return 1;
}
  ...
  for (i=0; i<node->c; i++)
	...
	if (node->v[i].subres->flags & FLAG_S)
	...
	else
	  {
		...
		ti->w = btn;
		XtAddCallback(btn, XmNvalueChangedCallback,
			      (XtCallbackProc)radio_callback, (XtPointer)ti);
	      }
	    else if (checked)
	      {
		if (strchr (checked, ','))
