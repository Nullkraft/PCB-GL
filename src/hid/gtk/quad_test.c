...
static void
quad_export_to_step (edge_ref e)
{
  FILE *f;
  time_t currenttime;
  struct tm utc;
  char *filename = "quad_test.step";
...
static edge_ref
quad_board_outline_to_quad (void)
{
  POLYAREA *outline;
  PLINE *contour;
  PLINE *ct;
...
void
quad_test_board_outline (void)
{
  edge_ref board_edges;

  board_edges = quad_board_outline_to_quad ();
//  printf ("\n");
//  printf ("Walking from board_edges\n");
//  quad_enum (board_edges, debug_print_edge, NULL);
  quad_export_to_step (board_edges);

  /* Replace our usual test cube with the board outline */
  quad_test_cube = board_edges;

  /* XXX: Leak the board outline anyway */
}
