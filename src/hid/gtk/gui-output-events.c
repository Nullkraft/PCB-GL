/* $Id: gui-output-events.c,v 1.1 2006-03-21 17:35:00 djdelorie Exp $ */
...
RCSID("$Id: gui-output-events.c,v 1.1 2006-03-21 17:35:00 djdelorie Exp $");
	...
	switch (ksym)
		...
		case GDK_K:
			if (mk == CONTROL_PRESSED || mk == SHIFT_CONTROL_PRESSED)
				{
				ghid_clear_increment_get_value(
		...
		case GDK_backslash:
			...
			ghid_set_menu_toggle_button(ghidgui->main_actions,
							"ToggleThinDraw", TEST_FLAG(THINDRAWFLAG, PCB));
			break;

		case GDK_Tab:
			hid_action ("SwapSides");
			ghid_set_menu_toggle_button(ghidgui->main_actions,
							"ToggleViewSolderSide", Settings.ShowSolderSide);
			break;
		...
		case GDK_period:
			...
			ghid_set_menu_toggle_button(ghidgui->main_actions,
						"Toggle45degree", TEST_FLAG (ALLDIRECTIONFLAG, PCB));
			break;
...
ghid_port_drawing_area_configure_event_cb (GtkWidget * widget,
			GdkEventConfigure * ev, GHidPort * out)
	{
