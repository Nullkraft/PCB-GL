...
draw_grid ()
{
  ...
  if (n > npoints)
    {
      npoints = n + 10;
      points = MyRealloc(points, npoints*sizeof(GdkPoint),"lesstif_draw_grid");
    }
  n = 0;
...
void ghid_invalidate_wh(int x, int y, 
			      int width, int height, int last)
{
	ghid_invalidate_all();
}

void ghid_invalidate_lr(int left, int right, 
			      int top, int bottom, int last)
{
	ghid_invalidate_all();
}
...
void ghid_invalidate_all()
{
  ...
  if (ebottom < gport->height)
    gdk_draw_rectangle (gport->drawable, gport->offlimits_gc,
         1, eleft, ebottom, eright-eleft+1, gport->height-ebottom);
  else
...
ghid_pinout_redraw (PinoutType *po)
{
  ...
  gdk_draw_rectangle(window, gport->bg_gc, TRUE,
...
int ghid_set_layer(const char *name, int group)
{
...
void ghid_use_mask(int use_it)
{
  ...
  case HID_MASK_CLEAR:
    ...
    gdk_draw_rectangle (gport->drawable, gport->mask_gc, TRUE, 0, 0, gport->width, gport->height);
    color.pixel = 0;
    gdk_gc_set_foreground (gport->mask_gc, &color);
    break;
...
void ghid_set_line_cap_angle(hidGC gc, int x1, int y1,
			       int x2, int y2)
{
...
ghid_draw_arc(hidGC gc, int cx, int cy,
	...
	if (   SIDE_X(cx) < gport->view_x0 - radius
	    ...
	    || cy < gport->view_y0 - radius
	    || cy > gport->view_y0 + h + radius
	   )
		return;
...
void ghid_fill_circle(hidGC gc, int cx, int cy,
		      int radius)
	{
	gint w, h, vr;
...
void ghid_fill_polygon(hidGC gc, int n_coords, 
			 int *x, int *y)
{
  static GdkPoint *points = 0;
  static int npoints = 0;
...
void ghid_extents_draw_line(hidGC gc, int x1, int y1,
		      int x2, int y2)
{
  printf("ghid_extents_draw_line()\n");
}

void ghid_extents_draw_arc(hidGC gc, int cx, int cy,
...
void ghid_extents_draw_rect(hidGC gc, int x1, int y1,
		      int x2, int y2)
{
  printf("ghid_extents_draw_rect()\n");
}

void ghid_extents_fill_circle(hidGC gc, int cx, int cy,
		      int radius)
{
  printf("ghid_extents_fill_circle()\n");
}

void ghid_extents_fill_polygon(hidGC gc, int n_coords, 
			 int *x, int *y)
{
  printf("ghid_extents_fill_polygon()\n");
}

void ghid_extents_fill_rect(hidGC gc, int x1, int y1,
		      int x2, int y2)
{
