/* $Id: gui-top-window.c,v 1.3 2006-03-22 22:49:30 danmc Exp $ */
...
RCSID("$Id: gui-top-window.c,v 1.3 2006-03-22 22:49:30 danmc Exp $");
...
handle_grid_units_change(gboolean active)
	...
	PCB->Grid = Settings.grid_units_mm
					? grid_mm_values[i] : grid_mil_values[i];
...
selected_lines_size_change_cb(GtkAction *action, GHidPort *port)
	...
	ghid_size_increment_get_value(gtk_action_get_name(action), &value, &units);
...
ghid_change_selected_update_menu_actions(void)
	...
	*/
	ghidgui->change_selected_actions = gtk_action_group_new("ChangeSelActions");
	gtk_action_group_set_translation_domain(ghidgui->change_selected_actions,NULL);
...
update_displayed_name_actions(void)
	...
	*/
	...
	gtk_action_group_set_translation_domain(ghidgui->displayed_name_actions, NULL);
	gtk_ui_manager_insert_action_group(ghidgui->ui_manager,
...
void
ghid_set_menu_toggle_button(GtkActionGroup *ag, gchar *name, gboolean state)
	{
	GtkAction	*action;
...
ghid_sync_with_new_layout(void)
	...
	*/
	...
	action = gtk_action_group_get_action(ghidgui->main_actions, "ToggleDrawGrid");
...
ghid_build_pcb_top_window (void)
{
  ...
  if (ghidgui->compact_horizontal)
    {
      gtk_box_pack_end (GTK_BOX (ghidgui->compact_vbox), ghidgui->position_hbox,
			FALSE, FALSE, 0);
    }
  else
    {
  ...
  */
  ...
  g_signal_connect(G_OBJECT(gport->drawing_area), "expose_event",
		   G_CALLBACK(ghid_port_drawing_area_expose_event_cb), port);
...
ghid_interface_input_signals_connect(void)
	...
	button_press_handler = 
		g_signal_connect(G_OBJECT(gport->drawing_area), "button_press_event",
				G_CALLBACK(ghid_port_button_press_cb), ghidgui->ui_manager);

	button_release_handler =
		g_signal_connect(G_OBJECT(gport->drawing_area), "button_release_event",
				G_CALLBACK(ghid_port_button_release_cb), ghidgui->ui_manager);

	key_press_handler =
		g_signal_connect(G_OBJECT(gport->drawing_area), "key_press_event",
				G_CALLBACK(ghid_port_key_press_cb), ghidgui->ui_manager);

	key_release_handler =
		g_signal_connect(G_OBJECT(gport->drawing_area), "key_release_event",
				G_CALLBACK(ghid_port_key_release_cb), ghidgui->ui_manager);
	}
...
ghid_init_icons(GHidPort *port)
	...
	XC_clock_mask = gdk_bitmap_create_from_data(gport->top_window->window,
						rotateMask_bits,
	...
	XC_hand_mask =  gdk_bitmap_create_from_data(gport->top_window->window,
						handMask_bits,
	...
	XC_lock_mask =  gdk_bitmap_create_from_data(gport->top_window->window,
						lockMask_bits,
