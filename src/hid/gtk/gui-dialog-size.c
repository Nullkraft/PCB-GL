/* $Id: gui-dialog-size.c,v 1.1 2006-03-21 17:35:00 djdelorie Exp $ */
...
RCSID("$Id: gui-dialog-size.c,v 1.1 2006-03-21 17:35:00 djdelorie Exp $");
...
via_hole_cb(GtkWidget *widget, SizesDialog *sd)
	...
	if (via_size < via_hole_size + FROM_PCB_UNITS(MIN_PINORVIACOPPER))
		gtk_spin_button_set_value(GTK_SPIN_BUTTON(sd->via_size_spin_button),
					via_hole_size + FROM_PCB_UNITS(MIN_PINORVIACOPPER));
	}
...
via_size_cb(GtkWidget *widget, SizesDialog *sd)
	...
	if (via_hole_size > via_size - FROM_PCB_UNITS(MIN_PINORVIACOPPER))
		gtk_spin_button_set_value(GTK_SPIN_BUTTON(sd->via_hole_spin_button),
					via_size - FROM_PCB_UNITS(MIN_PINORVIACOPPER));
	}
...
use_temp_cb(GtkWidget *button, gpointer data)
	...
	if (which == 1 && active)
		gtk_toggle_button_set_active(
			GTK_TOGGLE_BUTTON(route_sizes.set_temp2_button), FALSE);
	else if (which == 2 && active)
		gtk_toggle_button_set_active(
			GTK_TOGGLE_BUTTON(route_sizes.set_temp1_button), FALSE);
	}
...
ghid_route_style_dialog(gint index, RouteStyleType *temp_rst)
	...
	ghid_table_spin_button(table, 0, 0,
				...
				FROM_PCB_UNITS(MIN_LINESIZE), FROM_PCB_UNITS(MAX_LINESIZE),
				STEP0_SIZE, STEP1_SIZE,
				SPIN_DIGITS, 0, NULL,
				sd, TRUE, _("Line width"));
	ghid_table_spin_button(table, 1, 0,
				&sd->via_hole_spin_button,
				FROM_PCB_UNITS(rst->Hole),
				FROM_PCB_UNITS(MIN_PINORVIAHOLE),
				FROM_PCB_UNITS(MAX_PINORVIASIZE - MIN_PINORVIACOPPER),
				STEP0_SIZE, STEP1_SIZE,
				SPIN_DIGITS, 0, via_hole_cb,
				sd, TRUE, _("Via hole"));
	ghid_table_spin_button(table, 2, 0,
				&sd->via_size_spin_button,
				FROM_PCB_UNITS(rst->Diameter),
				FROM_PCB_UNITS(MIN_PINORVIAHOLE + MIN_PINORVIACOPPER),
				FROM_PCB_UNITS(MAX_PINORVIASIZE),
				STEP0_SIZE, STEP1_SIZE,
				SPIN_DIGITS, 0, via_size_cb,
				sd, TRUE, _("Via size"));
	ghid_table_spin_button(table, 3, 0,
				&sd->clearance_spin_button,
				FROM_PCB_UNITS(rst->Keepaway),
				FROM_PCB_UNITS(MIN_LINESIZE),
				FROM_PCB_UNITS(MAX_LINESIZE),
				STEP0_SIZE, STEP1_SIZE,
				SPIN_DIGITS, 0, NULL,
				sd, True, _("Clearance"));
	...
	if (!editing_temp)
		...
		ghid_check_button_connected(vbox1, &sd->set_temp1_button, FALSE,
				TRUE, FALSE, FALSE, 0,
				use_temp_cb, GINT_TO_POINTER(1), _("Temp1"));
		ghid_check_button_connected(vbox1, &sd->set_temp2_button, FALSE,
				TRUE, FALSE, FALSE, 0,
				use_temp_cb, GINT_TO_POINTER(2), _("Temp2"));

		vbox1 = ghid_category_vbox(vbox, _("Default Style"),
					4, 2, TRUE, TRUE);
		label = gtk_label_new("");
		gtk_label_set_use_markup(GTK_LABEL(label), TRUE);
		s = g_strdup_printf(_("<small>"
	...
	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_OK)
		...
		if (!editing_temp)
			{
			set_temp1 = gtk_toggle_button_get_active(
							GTK_TOGGLE_BUTTON(sd->set_temp1_button));
			set_temp2 = gtk_toggle_button_get_active(
							GTK_TOGGLE_BUTTON(sd->set_temp2_button));
			set_default =  gtk_toggle_button_get_active(
							GTK_TOGGLE_BUTTON(set_default_button));
			}
		if (set_temp1 || set_temp2)
			rst = &rst_buf;

		value = gtk_spin_button_get_value(
					GTK_SPIN_BUTTON(sd->line_width_spin_button));
		rst->Thick = TO_PCB_UNITS(value);

		value = gtk_spin_button_get_value(
					GTK_SPIN_BUTTON(sd->via_hole_spin_button));
		rst->Hole = TO_PCB_UNITS(value);

		value = gtk_spin_button_get_value(
					GTK_SPIN_BUTTON(sd->via_size_spin_button));
		rst->Diameter = TO_PCB_UNITS(value);

		value = gtk_spin_button_get_value(
					GTK_SPIN_BUTTON(sd->clearance_spin_button));
