/* $Id: gui-command-window.c,v 1.1 2006-03-21 17:35:00 djdelorie Exp $ */
...
RCSID("$Id: gui-command-window.c,v 1.1 2006-03-21 17:35:00 djdelorie Exp $");
	...
	for (i = 0, list = history_list; list; list = list->next, ++i)
		...
		if (!strcmp(cmd, s))
			...
			gtk_combo_box_remove_text(
						GTK_COMBO_BOX(ghidgui->command_combo_box), i);
			gtk_combo_box_prepend_text(
						GTK_COMBO_BOX(ghidgui->command_combo_box), s);
			return;
			}
		}
...
command_combo_box_entry_create(void)
	...
	ghidgui->command_entry = GTK_ENTRY(GTK_BIN(ghidgui->command_combo_box)->child);
...
ghid_command_window_show(void)
	...
	gtk_window_set_title(GTK_WINDOW(command_window),
				_("PCB Command Entry"));
	gtk_window_set_wmclass(GTK_WINDOW(command_window),
				"PCB_Command", "PCB");
...
ghid_command_entry_get(gchar *prompt, gchar *command)
	...
	*/
	gtk_window_remove_accel_group(GTK_WINDOW(out->top_window),
				gtk_ui_manager_get_accel_group(ghidgui->ui_manager));
	...
	escape_sig_id = g_signal_connect(G_OBJECT(ghidgui->command_entry),
				"key_press_event", G_CALLBACK(command_escape_cb), NULL);
