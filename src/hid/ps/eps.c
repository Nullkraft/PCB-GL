/* $Id: eps.c,v 1.4 2006-03-22 22:49:30 danmc Exp $ */
...
RCSID("$Id: eps.c,v 1.4 2006-03-22 22:49:30 danmc Exp $");
...
eps_get_export_options (int *n)
{
  ...
  if (PCB && PCB->Filename
      && eps_attribute_list[HA_psfile].default_val.str_value == last_made_filename)
    {
...
#define pcb2em(x) (int)((x) / 100000.0 * 72.0 * options[HA_scale].real_value + 1)
  ...
  lastcolor = -1;
#define Q 1000
  fprintf(f, "/nclip { %d %d moveto %d %d lineto %d %d lineto %d %d lineto %d %d lineto eoclip newpath } def\n",
