    ...
    ELEMENT_LOOP (PCB->Data);
      {
...
#ifdef REVERSED_PCB_CONTOURS
        instance->ox =                     ( ox * cos_rot + oy * sin_rot) + COORD_TO_STEP_X (PCB, element->MarkX);
        instance->oy = -on_solder_negate * (-ox * sin_rot + oy * cos_rot) + COORD_TO_STEP_Y (PCB, element->MarkY);
        instance->oz =  on_solder_negate * oz                             + (on_solder ? -COORD_TO_STEP_Z (PCB, HACK_BOARD_THICKNESS) : 0.0);
        ...
        instance->ox =                     ( ox * cos_rot + oy * sin_rot) + COORD_TO_STEP_X (PCB, element->MarkX);
        instance->oy =  on_solder_negate * (-ox * sin_rot + oy * cos_rot) + COORD_TO_STEP_Y (PCB, element->MarkY);
        instance->oz =  on_solder_negate * oz                             + on_solder_negate * -COORD_TO_STEP_Z (PCB, HACK_BOARD_THICKNESS) / 2.0;
        instance->ax =                     ( ax * cos_rot + ay * sin_rot);
        instance->ay =  on_solder_negate * (-ax * sin_rot + ay * cos_rot);
        instance->az =  on_solder_negate * az;
        instance->rx =                     ( rx * cos_rot + ry * sin_rot); /* XXX: SHOULD THIS FACTOR IN on_solder_negate? */
