...
static char *rcsid = "$Id: file.c,v 1.13 2004-03-17 04:59:44 haceaton Exp $";
  ...
  ELEMENT_LOOP (Data);
  {
    ...
    PAD_LOOP (element);
    {
      fprintf (FP, "\tPad[%i %i %i %i %i %i %i ",
	       ...
	       (int) pad->Thickness, (int) pad->Clearance, (int) pad->Mask);
    ...
    ARC_LOOP (element);
    {
      fprintf (FP,
	       ...
	       (int) arc->StartAngle, (int) arc->Delta, (int) arc->Thickness);
    }
    END_LOOP;
    fputs ("\n\t)\n", FP);
  }
