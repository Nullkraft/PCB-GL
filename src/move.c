...
static char *rcsid = "$Id: move.c,v 1.2 2003-12-25 16:27:13 haceaton Exp $";
...
{
  POLYGONPOINT_LOOP (Polygon, MOVE (point->X, point->Y, DeltaX, DeltaY);
    );
    ...
    LINE_LOOP (Layer,
		   ...
		   (via =
		    CreateNewVia (PCB->Data, new->Point1.X, new->Point1.Y,
				  ...
				  VIAFLAG)) != NULL)
	       {
	       UpdatePIPFlags (via, (ElementTypePtr) via,
			       NULL, NULL, False);
...
MovePolygonToLayer (LayerTypePtr Layer, PolygonTypePtr Polygon)
{
  ...
  VIA_LOOP (PCB->Data,
	    ...
	    CLEAR_FLAG (LayerThermFlag, via); SET_FLAG (DestThermFlag, via);}
  );
