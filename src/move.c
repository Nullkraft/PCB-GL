			 ...
			 LayerTypePtr Destination)
{
  ...
  *Text = Source->Text[--Source->TextN];
  ...
  ClearFromPours (PCB->Data, TEXT_TYPE, Destination, newone);
  return (newone);
}
...
MovePourToLayerLowLevel (LayerTypePtr Source, PourTypePtr Pour,
			    LayerTypePtr Destination)
{
  PourTypePtr newone = GetPourMemory (Destination);
  Cardinal i;
  ...
  *Pour = Source->Pour[--Source->PourN];
  ...
  r_insert_entry (Destination->pour_tree, (BoxType *) newone, 0);
  return (newone);
}
...
#endif
  newone = MovePourToLayerLowLevel (Layer, Pour, Dest);
  InitPourClip (PCB->Data, Dest, newone);
  if (Dest->On)
    {
      DrawPour (Dest, newone, 0);
      Draw ();
    }
  return (newone);
}
